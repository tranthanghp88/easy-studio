...
export default function App() {
  ...
  const [scriptFormat, setScriptFormat] = useState("Podcast 2 người nói - A/R");
...
{{SOURCE_TEXT}}`;
    ...
    setPatchInstruction("");
    setStatus("Đã AI Patch và cập nhật Script cuối. Kiểm tra lại trước khi export.");
    setRunning(null);
  }
  ...
  async function loadProject() {
    ...
    if (result?.ok && result.data && typeof result.data === "object") {
      ...
      setScriptFormat(data.scriptFormat || "Podcast 2 người nói - A/R");
  ...
  function goHome() {
    ...
    setStatus("Đã quay về Dashboard Easy Studio.");
  }
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <div className="topbar-title easy-studio-child-title">
      ...
      {showApiSettings && (
        <section className="settings-card api-settings-card creator-api-card">
          ...
          <div className="settings-actions">
            <button onClick={testProvider}>✓ Kiểm tra kết nối</button>
            <button className="primary" onClick={saveApiSettings}>
              💾 Lưu API
            </button>
          </div>
          <details className="advanced-settings" open={advancedMode}>
            <summary onClick={(event) => { event.preventDefault(); setAdvancedMode((value) => !value); }}>Cài đặt nâng cao</summary>
            ...
            <div className="prompt-chain-entry">
              ...
              <button className="btn-secondary" onClick={() => setShowPromptEditor(true)}>🧩 Xem / sửa Prompt</button>
            </div>
          </details>
        </section>
      )}
      ...
      <section className="settings-card compact-settings creator-quick-card">
        <div className="quick-copy">
          <span className="eyebrow">Chế độ tạo nội dung</span>
        ...
        <label>
          ...
          <select value={scriptFormat} onChange={(e) => setScriptFormat(e.target.value)}>
            <option value="Podcast 2 người nói - A/R">Podcast 2 người nói - A/R</option>
            <option value="Một người dẫn - NARRATOR">Một người dẫn - NARRATOR</option>
            <option value="Kể chuyện YouTube - VOICEOVER / SCENE">Kể chuyện YouTube - VOICEOVER / SCENE</option>
            <option value="Video giáo dục - HOOK / BODY / CTA">Video giáo dục - HOOK / BODY / CTA</option>
            <option value="Định dạng tùy chỉnh">Định dạng tùy chỉnh</option>
          </select>
        </label>
        <label>
          Chất lượng
      ...
      <section className="workspace script-workspace">
        ...
        <aside className="right-panel">
          ...
          <textarea
            ...
            placeholder="Nội dung script sẽ hiển thị tại đây..."
          />
          <details>
              ...
              <div className="patch-form">
                <label>
                  ...
                  <select value={patchScope} onChange={(event) => setPatchScope(event.target.value as "final" | "active")}>
                    <option value="final">Script cuối</option>
              ...
              <div className="patch-preview">
                <div className="panel-head">
                  <div>
                    ...
                    <p>{patchScope === "final" ? "Script cuối" : steps.find((s) => s.id === activeStep)?.title}</p>
                  </div>
