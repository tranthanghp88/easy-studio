...
export function useKeyManager() {
  ...
  async function handleImportKeys(file: File) {
    ...
    const normalizedRes = await apiFetch("/api/keys/normalize", {
      method: "POST"
    });
    const normalizedData = await normalizedRes.json().catch(() => ({}));
    if (!normalizedRes.ok) {
      throw new Error(normalizedData?.error || "Import xong nhưng chuẩn hóa key thất bại");
    }

    await fetchDashboardData();
    return normalizedData?.totalKeys != null
      ? { ...data, totalKeys: normalizedData.totalKeys, normalized: true }
