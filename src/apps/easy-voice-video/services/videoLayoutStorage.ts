export type VideoLayoutSettings = {
  subtitle: {
    ...
    karaokeMode: "off" | "speakerToWhite" | "whiteToSpeaker";
  };
  wavebar: {
    xOffset: number;
    y: number;
...
export const DEFAULT_VIDEO_LAYOUT_SETTINGS: VideoLayoutSettings = {
  subtitle: {
    ...
    colorA: "#37A5B4", colorR: "#BE6E55", colorBoth: "#C8AA5A", outlineColor: "#353535", backgroundColor: "rgba(0,0,0,0.18)", karaokeMode: "speakerToWhite"
  },
  wavebar: {
    ...
    glowStrength: 0.75, peakMarker: false, mirrorMode: false, waveStyle: "podcast",
...
function mergeSettings(raw: any): VideoLayoutSettings {
  ...
  return {
    subtitle: {
      ...
      karaokeMode: (["off", "speakerToWhite", "whiteToSpeaker"].includes(String(s?.subtitle?.karaokeMode)) ? String(s.subtitle.karaokeMode) : DEFAULT_VIDEO_LAYOUT_SETTINGS.subtitle.karaokeMode) as "off" | "speakerToWhite" | "whiteToSpeaker"
    },
    wavebar: {
