const { WAVEBAR_RENDER_CONFIG } = require("./video/wavebar-config.cjs");
const { buildStyledAssContent } = require("./video/subtitle-layout.cjs");
const { processTimeline } = require("./video/timelineProcessorService.cjs");
...
ipcMain.handle("app:get-update-status", async () => ({
  ...
  percent: 0,
  error: "",
  message: "Auto update chưa bật trong bản dev này."
}));

ipcMain.handle("app:check-for-updates", async () => ({
  ok: false,
  error: "Auto update chưa bật trong bản dev này."
}));
...
ipcMain.handle("file:merge-video-files", async (_event, payload = {}) => {
  const tempFiles = [];
  try {
...
ipcMain.handle("file:list-laugh-assets", async () => {
  try {
...
ipcMain.handle("file:import-laugh-assets", async (_event, payload = {}) => {
  try {
    const files = Array.isArray(payload.files) ? payload.files.map((x) => safeText(x)).filter(Boolean) : [];
...
ipcMain.handle("file:delete-laugh-asset", async (_event, payload = {}) => {
  try {
...
ipcMain.handle("file:open-laugh-assets-folder", async () => {
  try {
    const root = ensureLaughAssetsRootDir();
...
ipcMain.handle("file:read-base64", async (_event, payload = {}) => {
  try {
    const filePath = safeText(payload.filePath);
