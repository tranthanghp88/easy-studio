                ...
                <>
                  <div className="presetCompactList">
                    {blockPresets.map((preset) => {
                      ...
                      const settings = (preset.speakerSettings || {}) as Partial<SpeakerSettings>;
                      const speakerA = (settings.A || {}) as Partial<SpeakerPreset>;
                      const speakerR = (settings.R || {}) as Partial<SpeakerPreset>;
                      return (
                        <React.Fragment key={preset.id}>
                          <div className={`presetCompactRow ${isDetail ? "active" : ""}`}>
                            <button
                              type="button"
                            ...
                            <button
                              ...
                              onClick={() => setExpandedPresetIds(isDetail ? [] : [preset.id])}
                            >
                              Chi tiết
                            </button>
                            <button
                            ...
                            <label className="presetCheckWrap" title="Tick chọn preset">
                              <input
                                type="checkbox"
                                checked={isChecked}
                                onChange={() => handleSelectPresetForDelete(preset.id)}
                              />
                            </label>
                          </div>

                          {isDetail ? (
                            <div className="presetInlineDetail">
                              <div>
                                Speed A: {formatNumber(Number(speakerA.speed ?? 1))} - Pitch A: {formatNumber(Number(speakerA.pitch ?? 0))} - Pause A: {formatNumber(Number(speakerA.pause ?? 0))}
                              </div>
                              <div>
                                Speed R: {formatNumber(Number(speakerR.speed ?? 1))} - Pitch R: {formatNumber(Number(speakerR.pitch ?? 0))} - Pause R: {formatNumber(Number(speakerR.pause ?? 0))}
                              </div>
                              <div>
                                Pause Block: {formatNumber(Number(settings.blockPause ?? 0))}
                              </div>
                            </div>
                          ) : null}
                        </React.Fragment>
