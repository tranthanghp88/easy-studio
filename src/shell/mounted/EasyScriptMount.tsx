import scriptCss from "../../apps/easy-script/src/style.css?inline";
import ScriptApp from "../../apps/easy-script/src/App.mount";
import { useMountedCss } from "./useMountedCss";

const scriptShellCss = `
.easy-script-shell-host {
  min-height: 100vh;
  width: 100%;
  overflow: auto;
  background:
    radial-gradient(circle at top left, rgba(47, 103, 255, .16), transparent 34%),
    radial-gradient(circle at top right, rgba(22, 168, 107, .10), transparent 30%),
    linear-gradient(180deg, #f7f9ff 0%, #eef3fb 48%, #edf2fb 100%);
  color: #142033;
}
.easy-script-shell-host, .easy-script-shell-host * { color-scheme: light; }
.easy-script-shell-host .app-shell {
  min-height: 100vh;
}
...
export default function EasyScriptMount() {
  // Keep Script app close to its standalone UI: original CSS + same light background.
  useMountedCss(scriptCss + "\n" + scriptShellCss, "easy-script-standalone-css");
  return (
    <div className="easy-script-shell-host">
      <ScriptApp />
    </div>
  );
}
