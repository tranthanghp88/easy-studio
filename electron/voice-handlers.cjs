  ...
  if (!musicBeds.length) {
    ...
    if (!validBeds.length) {
      ...
      validBeds.forEach((bed) => {
        if (bed.loop) audioArgs.push("-stream_loop", "-1");
        audioArgs.push("-i", safeText(bed.filePath));
      });
      ...
      validBeds.forEach((bed, idx) => {
        ...
        const remaining = Math.max(0.1, audioDurationAfterConcat - start);
        const requestedDuration = ensureNumber(bed.duration, 0);
        const duration = Math.min(remaining, Math.max(0.1, requestedDuration || remaining));
        const fadeOut = Math.min(duration / 2, Math.max(0, ensureNumber(bed.fadeOut, 2)));
        const fadeStart = Math.max(0, duration - fadeOut);
        const fadeFilter = fadeOut > 0.05 ? `,afade=t=out:st=${fadeStart.toFixed(3)}:d=${fadeOut.toFixed(3)}` : "";
        filterParts.push(
          `[${inputIndex}:a]aformat=sample_fmts=fltp:sample_rates=24000:channel_layouts=mono,atrim=0:${duration.toFixed(3)},asetpts=PTS-STARTPTS${fadeFilter},adelay=${Math.round(start * 1000)}|${Math.round(start * 1000)},volume=${volume.toFixed(3)}[bgm${idx}]`
        );
      });
      ...
      filterParts.push(`[voicebase][bgmduck]amix=inputs=2:normalize=0:duration=first:dropout_transition=0[aout]`);

      audioArgs.push(
        "-filter_complex", filterParts.join(";"),
        "-map", "[aout]",
        "-t", audioDurationAfterConcat.toFixed(3),
        "-c:a", "pcm_s16le",
        finalAudioPath
      );
